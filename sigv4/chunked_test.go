...
func TestChunkedReaderAWSDocsVector(t *testing.T) {
	r := sigv4.NewChunkedReader(bytes.NewReader(awsDocsChunkedBody()), awsDocsVerifiedRequest(), "", 66560)
...
func TestChunkedReaderBadSignature(t *testing.T) {
	...
	r := sigv4.NewChunkedReader(bytes.NewReader(corrupted), awsDocsVerifiedRequest(), "", 66560)
	_, err := io.ReadAll(r)
	if err == nil {
...
func TestChunkedReaderTamperedData(t *testing.T) {
	...
	body[i] = 'b'
	r := sigv4.NewChunkedReader(bytes.NewReader(body), awsDocsVerifiedRequest(), "", 66560)
	_, err := io.ReadAll(r)
	if err == nil {
...
func TestChunkedReaderRoundTrip(t *testing.T) {
	...
	for _, size := range []int{1, 7, 1000, 65536} {
		t.Run(fmt.Sprintf("chunk size %d", size), func(t *testing.T) {
			...
			r := sigv4.NewChunkedReader(bytes.NewReader(encoded), vr, "", int64(len(data)))
			decoded, err := io.ReadAll(r)
			if err != nil {
				t.Fatal(err)
			}
...
func TestChunkedReaderUnsignedTrailer(t *testing.T) {
	...
	r := sigv4.NewChunkedReader(buf, vr, "", int64(len(data)))
	decoded, err := io.ReadAll(r)
	if err != nil {
		t.Fatal(err)
	}
...
func TestChunkedReaderTrailerChecksum(t *testing.T) {
	...
	t.Run("sha256 roundtrip", func(t *testing.T) {
		...
		r := sigv4.NewChunkedReader(buf, vr, "sha256", int64(len(data)))
