...
- Policies are tenant-authored and evaluated on every request (per object for DeleteObjects), so they are untrusted input with structural caps enforced at parse/validate time (`policy.Max*`: 20 statements, 20 actions, 20 resources, 128-byte patterns) — the glob matcher is O(n·m) (no ReDoS), but unbounded size would still let one authorization do unbounded work. `Match` converts the value to runes once per `Evaluate`/`Allows` (not per pattern) since the same key is matched against every statement.
