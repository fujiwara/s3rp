...
- Backend clients are built lazily and cached by backend identity (endpoint/region/credentials/path-style); clients are bucket-agnostic. The cache key must keep covering exactly the fields `newBackendClient` reads — the bucket is deliberately excluded, everything else must be present or differently-configured backends would silently share a client.
- Optional backend fields are resolved by `store.Backend.SetDefaults(bucketName)`, which **every** Store implementation must call (the config store at load time, `store/rdb` after unmarshaling a row). DB rows may legitimately omit `use_path_style`/`region`/`bucket` — they are `omitempty` — and the client builder used to dereference `*UsePathStyle` unconditionally, so a row written by another tool panicked the request path.
...
- Policies are tenant-authored and evaluated on every request (per object for DeleteObjects), so they are untrusted input with structural caps enforced at parse/validate time (`policy.Max*`: 20 KB total document, 20 statements, 30 actions, 10 resources, 128-byte patterns, 100 principal users; the byte cap is checked before unmarshal so an oversized document is rejected without being parsed into memory). The worst-case cost is dominated by the total resource-pattern count (`MaxStatements*MaxResourcesPerStatement`) — each adversarial resource pattern can force a full scan of the object key — not by pattern length or the byte cap; action matching is against a short string and stays cheap, so `MaxActionsPerStatement` is generous. — the glob matcher is O(n·m) (no ReDoS), but unbounded size would still let one authorization do unbounded work. Perf: patterns are compiled to `[]rune` once (lazily via `sync.Once` on first `Evaluate`/`Allows`, so `Policy`/`UserPolicy` carry a lock and must be passed by pointer — `db.marshalUserPolicy` marshals `&UserPolicy`), and the matched value is converted to runes once per call rather than per pattern; together this keeps a maxed-out policy's evaluation in the low microseconds. DeleteObjects authorizes per object but resolves the resource-independent parts once via `app.perObjectAuthorizer` / `policy.DenyEvaluatorFor` (user policy + the bucket policy's matching Deny statements), so only the resource is tested per key; when no Deny matches the user and action (the common case), `perObjectAuthorizer.allowsEverything` (backed by `DenyEvaluator.AlwaysAllows`) skips the per-key check and its resource-string construction entirely — de-amplifying the ×1000 to O(1). Only a policy with resource-discriminating Deny rules for the action pays per key, bounded by the resource-count cap.
  ...
  Caveat on where the precompilation pays off: `configStore` builds each `Policy`/`UserPolicy` once at startup, so patterns are compiled once for the process. `store/rdb` has no cache and re-parses (and therefore re-compiles) per request, so there the win is only within a single request's evaluations. Keep this in mind when reading the benchmarks — they measure a warm, long-lived policy.
- The user-policy byte cap (`MaxPolicyBytes`) applies to the **serialized** form, so it lives in `policy.MarshalUserPolicy` (used by config validation and `db.Import`), not in `ValidateUserPolicy`. `store/rdb.GetKey` checks the raw stored string directly instead, keeping `ValidateUserPolicy` allocation-free on the per-request path — do not move the cap back into it.
...
- **One signer per access key** (`signer.go`). `v4.Signer`'s internal derived-key cache holds a *single* entry keyed by (service, region) and validates it against the request's access key id — right for an SDK client with one credential, wrong for a proxy that signs with a different key almost every request: every request misses, re-derives the signing key, and takes that cache's **write lock**, so verification serializes process-wide (measured: with 16 keys the parallel case ran *slower* than serial, 16.4µs vs 13.8µs). `signerCache` is a fixed-size direct-mapped table (`signerShards` slots, collisions just replace the occupant) so memory stays bounded — access keys are per user and can grow without limit, unlike backends. Guarded by `BenchmarkVerifyKeyDiversity`.
...
- Error responses: S3 XML via `S3Error` — an alias of `s3err.Error`; the root shim in error.go keeps the operation layer's call sites unqualified (`s3err.FromSDKError` preserves backend code/status; `wellKnownErrorStatus` maps code-only errors). HEAD and 304 responses must not carry a body. Every response gets `x-amz-request-id`.
