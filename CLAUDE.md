...
- Policies are tenant-authored and evaluated on every request (per object for DeleteObjects), so they are untrusted input with structural caps enforced at parse/validate time (`policy.Max*`: 20 KB total document, 20 statements, 20 actions, 20 resources, 128-byte patterns, 100 principal users; the byte cap is checked before unmarshal so an oversized document is rejected without being parsed into memory) — the glob matcher is O(n·m) (no ReDoS), but unbounded size would still let one authorization do unbounded work. Perf: patterns are compiled to `[]rune` once (lazily via `sync.Once` on first `Evaluate`/`Allows`, so `Policy`/`UserPolicy` carry a lock and must be passed by pointer — `db.marshalUserPolicy` marshals `&UserPolicy`), and the matched value is converted to runes once per call rather than per pattern; together this keeps a maxed-out policy's evaluation in the low microseconds.
