...
`POST /bucket` with a `multipart/form-data` body and a [SigV4 POST policy](https://docs.aws.amazon.com/AmazonS3/latest/API/sigv4-UsingHTTPPOST.html) is supported: a backend service signs a policy document with a front-side key, and the browser uploads directly to s3rp with an HTML form — no credentials in the page. Combined with [CORS](#cors), this is the standard browser-direct upload flow, and forms generated by the AWS SDK presigned-POST helpers work as they are.
